...
greenLow = np.array([70, 90, 50], np.uint8)
greenHigh = np.array([90, 255, 255], np.uint8)
...
def color_detector(mask, color, name):
    global point_cloud, err, data_dict

    (cnts, _) = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    for c in cnts:
        area = cv2.contourArea(c)
        if area > 3000 and area < 28000:
            M = cv2.moments(c)
            if (M["m00"] == 0):
                M["m00"] == 1
            x = int(M["m10"]/M["m00"])
            y = int(M["m01"]/M["m00"])
            smoothCont = cv2.convexHull(c)
            cv2.drawContours(image_net, [smoothCont], 0, color, 3)
            cv2.circle(image_net, (x, y), 5, (0, 0, 0), 1, cv2.LINE_AA)
            err, pc_value = point_cloud.get_value(x, y)
            if pc_value[0] == float('inf') or pc_value[0] == float('-inf'):
                pc_value[0] = float('nan')
            if not np.isnan(pc_value[0]):
                x_mm = round(pc_value[0])
                y_mm = round(pc_value[1])
                z_mm = round(pc_value[2])
                cv2.putText(image_net, f"{x_mm},{y_mm},{z_mm}",
                            (x+10, y), font, 0.75, (0, 0, 0), 1, cv2.LINE_AA)
                data_dict[name] = [x_mm, y_mm, z_mm]
...
def torch_thread(weights, img_size, conf_thres=0.7, iou_thres=0.45):
...
def main():
    ...
    viewer = gl.GLViewer()
    point_cloud_res = sl.Resolution(min(camera_infos.camera_resolution.width, 1280),
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            # -- Get the image
            lock.acquire()
            zed.retrieve_image(image_left_tmp, sl.VIEW.LEFT)  # LEFT)
            image_net = image_left_tmp.get_data()
            lock.release()
            run_signal = True

            HSV_image = cv2.cvtColor(image_net, cv2.COLOR_BGR2HSV)

            mask_blue = cv2.inRange(HSV_image, blueLow, blueHigh)
            mask_green = cv2.inRange(HSV_image, greenLow, greenHigh)
            mask_red1 = cv2.inRange(HSV_image, redLow1, redHigh1)
            mask_red2 = cv2.inRange(HSV_image, redLow2, redHigh2)
            ...
            if objects.object_list:
                for object in objects.object_list:
                    pos = object.position
                    pos[np.isnan(pos)] = 0
                    data_dict[object.raw_label] = pos.tolist()

            # -- Display

            # Retrieve display data
            zed.retrieve_measure(point_cloud, sl.MEASURE.XYZRGBA,
                                 sl.MEM.CPU, point_cloud_res)  # point_cloud_res
            color_detector(mask_blue, (255, 0, 0), "blue")
            color_detector(mask_green, (0, 100, 0), "green")
            color_detector(mask_red, (0, 0, 255), "red")
            ...
            cv2.imshow("Object detection", image_left_ocv)
            cv2.imshow("Color identification", image_net)
            key = cv2.waitKey(10)
            if key == 27:
                exit_signal = True
        else:
